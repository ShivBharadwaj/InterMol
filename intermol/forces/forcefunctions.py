...
def build_paramlist(program):
    ...
    change_list = eval(f'forcedata.{program}_paramlist')
    ...
    for name, paramset in tmp_paramlist.items():
        ...
        paramlist[capifyname(f'{name}_type')] = tmp_paramlist[name]
...
def get_parameter_list_from_kwds(force, kwds, paramlist):
    ...
    name = force.__class__.__name__
    return [kwds[p] for p in paramlist[name]]
...
def create_kwds_from_entries(unitvars, paramlist, entries, force_type, offset=0):
    ...
    return {p: float(entries[offset+i]) * u[i] for i, p in enumerate(params)}
...
def optforceparams(force_type, forcetype_object=None):
    ...
    """
    return {
        param: eval(f'{forcetype_object.__class__.__name__}.{param}')
        if forcetype_object
        else eval(forcedata.AbstractOptParamsDefaults[force_type][i])
