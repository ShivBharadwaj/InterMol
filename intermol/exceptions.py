...
class UnsupportedFunctional(ConversionError):
    ...
    def __str__(self):
        return f"The {self.could_not_convert.__class__.__name__} interaction type is not supported in {self.engine.upper()}."
...
class UnimplementedFunctional(ConversionError):
    ...
    def __str__(self):
        return f'{self.could_not_convert.__class__.__name__} conversion has not yet been implemented in InterMol for {self.engine.upper()}.'


class UnsupportedSetting(ConversionError):
    """Any setting that is not supported in a specific engine."""
    def __str__(self):
        return f"{self.could_not_convert} is not supported in {self.engine.upper()}."


class UnimplementedSetting(ConversionError):
    """Any setting that is not yet implemented in intermol for a specific engine."""
    def __str__(self):
        return f'{self.could_not_convert} has not yet been implemented in InterMol for {self.engine.upper()}.'
